...
### Karakter Sudokusu (`/character-sudoku`)
...
Çeviriler (TR/EN) `js/translations.js` içinde `eglence.*`, `nl.*`, `cs.*`, `tkz.*`, `crown.*`, `yapboz.*` anahtarlarıyla yönetilir.
