...
### Dahis Blok Yerleştirme (`/blok-yerlestirme`)
- **Tür:** Blok yerleştirme bulmacası (Block Blast tarzı), 8×8 ızgara.
- **Amaç:** Her turda verilen 3 neon Tetris-benzeri bloğu ızgaraya yerleştir. Dolu satır veya sütun temizlenir ve puan kazanırsın. Blok sığmazsa oyun biter.
- **Kurallar:**
  - Bir blok seç (blok tepsisine tıkla), ardından ızgarada istediğin konuma tıklayarak yerleştir.
  - Dolu satır veya sütun otomatik temizlenir (+10 puan/satır-sütun, çoklu temizleme bonusu).
  - Tüm 3 blok kullanıldığında yeni 3 blok gelir (+15 bonus).
  - Hiçbir blok tahtaya sığmazsa oyun biter.
- **Kontroller:** Fare tıklaması veya dokunma ile blok seç ve yerleştir; **Geri Al**, **Yeni Oyun** düğmeleri.
- **Görsel:** Parlak neon renkler, hover önizleme, satır/sütun temizleme animasyonu.
- **Dosyalar:** `blok-yerlestirme/index.html`, `blok-yerlestirme.css`, `blok-yerlestirme.js`.
...
└── blok-yerlestirme/     # Dahis Blok Yerleştirme (neon blok bulmacası)
└── zamanlama/            # Zamanlanmış Karakter Yerleştirme (saat temalı tepki oyunu)
```

Çeviriler (TR/EN) `js/translations.js` içinde `eglence.*`, `nl.*`, `cs.*`, `tkz.*`, `crown.*`, `yapboz.*`, `oneline.*`, `crystal.*`, `glitch.*`, `dahispath.*`, `blok.*`, `zkr.*` anahtarlarıyla yönetilir.
